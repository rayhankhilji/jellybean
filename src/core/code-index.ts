...
import { detectLanguage, isStructured } from '../lang/registry.js';
import type { CodeSymbol, ImportRef, LanguageId } from '../lang/types.js';
import { tokenizeCode } from '../util/text.js';
...
export class CodeIndex {
  ...
  /**
   * Bring the index up to date. Cheap to call on every tool invocation: a scan
   * that ran within the freshness window is a no-op, and concurrent callers
   * share a single in-flight scan.
   */
  async ensureFresh(force = false): Promise<void> {
    if (!force && Date.now() - this.lastScan < FRESHNESS_MS) return;
  ...
  private async scan(): Promise<void> {
    ...
    for (const entry of stale) await this.indexFile(entry, this.files.get(entry.path));
  ...
  private async indexFile(entry: WorkspaceFile, existing: FileRecord | undefined): Promise<void> {
    ...
    if (text === null) {
      ...
      if (isStructured(language)) {
        try {
          record.symbols = extractSymbols(text, language);
          record.imports = extractImports(text, language);
        } catch {
  ...
  private indexTerms(record: FileRecord, text: string): void {
    const frequencies = new Map<string, number>();
    for (const term of tokenizeCode(text)) {
      const current = frequencies.get(term);
      if (current === undefined) {
        if (frequencies.size >= MAX_TERMS_PER_FILE) continue;
